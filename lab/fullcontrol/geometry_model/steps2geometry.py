...
def generate_stl(data: PlotData, controls: ModelControls):
    ...
    for path in data.paths:
        if path.extruder.on:
            meshes.append(generate_mesh(path, 0, Mesh,
                          sides, rounding_strength, flat_sides))
...
def geometry_model(steps: list, model_controls: ModelControls = ModelControls()):
    ...
    model_controls.stl_filename += datetime.now().strftime(
        "__%d-%m-%Y__%H-%M-%S.stl") if model_controls.include_date == True else '.stl'
