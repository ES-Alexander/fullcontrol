...
def generate_mesh(path, linewidth_now: float, Mesh: FlowTubeMesh, sides, rounding_strength, flat_sides, colors_now: list = None):
    # this function is used in plot() in this module and also by generate_stl() in lab.fullcontrol.geometry_model.steps2geometry
...
def plot(data: PlotData, controls: PlotControls):
    ...
    if controls.style is None:  # if plot style not set by user, set to tubes, but print warning message to user
    ...
    for path in data.paths:
        ...
        if path.extruder.on and controls.style == 'tube':
            ...
            mesh = generate_mesh(path, linewidth_now, Mesh,
                                 sides, rounding_strength, flat_sides, colors_now)
